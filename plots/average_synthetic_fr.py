...
class Plot(BasePlot):
    ...
    name = "avg_synthetic_fr"
    type = "scatter"  # or "bar_chart", "boxplot" or "table"
    options = { # Will generate one plot per option entry !!!
        "dataset_param": [{'estimated_rank':10,'low_rank':False,'m_dim':200,'n_dim':200,'noise_type':'poisson','noisy':True,'snr':100,'true_rank':10,'sparsity_factors':1},
                          {'estimated_rank':20,'low_rank':False,'m_dim':500,'n_dim':500,'noise_type':'poisson','noisy':True,'snr':100,'true_rank':20,'sparsity_factors':1}],
        "objective": ...,
        "objective_column": ...,
    }
    ...
    def get_metadata(self, df, objective, objective_column, dataset_param):
        M = dataset_param["m_dim"]
        N = dataset_param["n_dim"]
        R = dataset_param["estimated_rank"]
        return {
            "title": f"(M,N,R)=({M},{N},{R})",
            "xlabel": "Time",
            "ylabel": "KL loss",
