...
class Solver(BaseSolver):
    """
    Multiplicative Updates with Burg entropy from "A Descent Lemma Beyond Lipschitz Gradient Continuity: First-Order Methods Revisited and Applications" 
    by Bauschke, H. H. and Bolte, J. and Teboulle, M.
    """
    name = "mu_burg"
    ...
    def run(self, callback):
        ...
        linegammaH = 1/(self.X.sum(axis=0)) #size n
        colgammaW = 1/(self.X.sum(axis=1)) #size m
        gammaH =  np.tile(linegammaH,(rank,1))
        gammaW = np.tile(colgammaW,(rank,1)).T
