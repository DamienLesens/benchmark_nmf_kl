...
class Solver(BaseSolver):
    """
    Primal-Dual algorithm from "Primal-Dual Algorithms for Non-negative Matrix Factorization with the Kullback-Leibler Divergence"
    by Yanez, F. and Bach, F.
    """
    name = "FPA"
    ...
    def run(self, callback):
        ...
        while callback():
            ...
            for _ in range(n_inner_iter):
                ...
                self.H = np.maximum(self.H - (self.W.T @ (chi + 1)) * tau, eps)
                Hbar = 2 * self.H - Hold
                Hold = self.H.copy()
            ...
            for _ in range(n_inner_iter):
                ...
                self.W = np.maximum(self.W - (chi + 1) @ self.H.T * tau, eps)
                Wbar = 2 * self.W - Wold
                Wold = self.W.copy()
